...
- GitHub Dependabotより以下のアラートが報告されている（すべてopen）
  - **high**: vite #33 #34（開発サーバーをネットワーク公開時のファイル読み取り・`fs.deny`バイパス）、black #25（任意ファイル書き込み）
  - **medium**: vite #35（`.map`パストラバーサル）、axios #41 #42（SSRF・Cloud Metadata漏洩）、follow-redirects #40（カスタム認証ヘッダー漏洩）、python-multipart #39（DoS）、pytest #38（tmpdir処理）
  - **low**: Pygments #32（ReDoS）
...
| 項番 | 項目 | 期待する動作 | 結果 |
|-----|------|------------|------|
| N-01 | `uv sync` が正常終了する | エラーなく依存解決される | - |
| N-02 | `uv pip list` でインストール済みバージョンを確認 | `black>=26.3.1`・`pytest>=9.0.3` を満たしている | - |
| N-03 | `pytest` が全件パスする | テストがすべてグリーン | - |
| N-04 | `npm run build` が正常終了する | フロントエンドビルドが成功する | - |
| N-05 | `npm list vite axios` でインストール済みバージョンを確認 | `vite>=7.3.2`・`axios>=1.15.0` を満たしている | - |
| E-01 | `rich` 削除後にCLIが正常起動する | `ImportError` が発生しない | - |
| E-02 | `python-multipart` 削除後にFastAPI起動が正常に行われる | アプリ起動時にエラーが出ない | - |
